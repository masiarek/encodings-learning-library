...
The Python library runs the same experiment as part of `open()`'s `buffering` argument, in [section 4 of its Opening a file ↗](https://masiarek.github.io/python-learning-library/01_Text_and_Bytes/opening_a_file/index.html#who-decides-when-the-output-leaves): the same inversion against a real pty, then the same question asked of a file you opened yourself — including why `buffering=0` is refused in text mode.
