...
- **[Donald A. Tevault, *The Ultimate Linux Shell Scripting Guide* (2024) ↗](https://openlibrary.org/isbn/9781835463574).** A broad bash book: text-stream filters, `grep` and `sed`, `awk`, here documents, user interfaces with `dialog`, portability, and scripts for security work. This library has checked one section of it, "Using od" in chapter 7. The commands there are right and the reading of the bytes is not. Its dumps show a UTF-8 BOM the text never names and `-t a` output only GNU `od` prints, and it credits `-j0000640`'s octal to the address column instead of the leading zero. [Reading a textbook's `od` session](../../11_Tools/od/README.md#reading-a-textbooks-od-session) goes through all five points. Read it for the shell, and check anything it says about bytes against [11_Tools](../../11_Tools/README.md). Its pipeline, redirection, `tee`, `grep` and prompt chapters are measured on a Mac and in fish by the [Linux learning library ↗](https://masiarek.github.io/linux-learning-library/).
