...
| 20 | [Ghidra's entropy bar: the encoding shows in the histogram](entropy/README.md) | Which encoding is this region of the file in, without decoding a byte — and what can a histogram not tell? | written, 2026-09-13 |
| 21 | [`uni` — the character's name](uni/README.md) | What *is* this character, not just how is it stored? | written |
| 22 | [`uni -h`, line by line](uni_help/README.md) | What does every word on `uni`'s help screen mean — and where does the program not do what it says? | written, 2026-09-10 |
| 23 | [Typing a character you cannot type](typing_a_character/README.md) | There is no `ż` on my keyboard — how do I produce one? | written, 2026-09-07 |
| 24 | [The five worth installing](worth_installing/README.md) | What do `hexyl`, `uchardet`, `recode`, `dos2unix` and GNU coreutils add? | written |
...
| **[Ghidra's entropy bar](entropy/README.md)** | colour a program by how evenly each kilobyte's bytes are spread | a **histogram** per chunk and never a decode: English scores about 4.2 in UTF-8 because bit 7 is never set, 3.1 in UTF-16 because every other byte is `00`, and compressed or encrypted data near 8.0. Blind to order, to repetition and to any one-to-one substitution, so [an EBCDIC string table scores as `ascii`](entropy/README.md#in-python) |
| [`hexyl`](worth_installing/README.md) | dump, in colour | colour by byte category, which is the one column no other dump has |
| [Hex Fiend ↗](https://hexfiend.com/) | view and edit the bytes, in a window (macOS); `hexf` opens a file in it | the one dump whose text column's encoding is a **menu** — ASCII to start with, then Mac OS Roman, Latin-1, Latin-2 and both UTF-16 orders, with UTF-8 added through *Customize…* — rather than a decision made for you. [A screenshot, read column by column](../06_Terminal/binary_or_text/README.md#looking-at-the-bytes) |
...
Every other chapter's claims are backed by a program CI runs on Ubuntu *and* macOS. Every page here keeps that contract for what it can, and four of them — [`ripgrep`](ripgrep/README.md), [`uni`](uni/README.md), [its help screen](uni_help/README.md) and [the five worth installing](worth_installing/README.md) — are about tools neither operating system ships, so CI has none of them and no answer key can be recorded from the tool itself. Those pages put the tool's real output in a fence that is **labelled and dated with the two machines it was measured on**, and keep a machine-checked example beside it doing the same job in the standard library — so the claim is still tested, just not by the tool it is about. [Ghidra's entropy bar](entropy/README.md) keeps the same contract for a tool neither runner has either: its arithmetic is three checked programs, its constants are read off the source at the installed tag, and the real files are in a dated fence.
