...
## Reading a textbook's `od` session

Most Linux books teach `od` the same way, and Donald A. Tevault's [*The Ultimate Linux Shell Scripting Guide* ↗](https://openlibrary.org/isbn/9781835463574) (Packt, 2024) is a good one to read against this page. Its chapter 7, "Using od", dumps `/bin/echo` in octal and in hex, then runs a Project Gutenberg text file through `-t c` and `-t a` to find its carriage returns and tabs. Every command in it works. The dumps also show five things about encoding that the text never mentions, and each of them has a page here. The run below builds files of the same shape (a text file with a BOM and CRLF lines, and the first eight bytes of an ELF file) and asks the questions the chapter skips.

<!-- output:od_textbook_session_sh -->
*Verified output of [`od_textbook_session_sh.sh`](examples/od_textbook_session_sh.sh) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. THREE BYTES BEFORE THE FIRST LETTER

$ od -tc book.txt | tidy
0000000 357 273 277 C h a p t e r \r \n \t I n d
0000020 e n t e d \r \n
0000027
   -t c prints a byte it cannot show as a character in octal. 357 273 277
   is not part of the title. In hex it is:

$ od -An -N3 -tx1 book.txt | tidy
ef bb bf
   ef bb bf, the UTF-8 encoding of U+FEFF: a byte order mark. And each
   line ends \r \n -- carriage return FIRST, then newline, the order a
   Windows editor and every CRLF protocol write them in. \t is the tab.

2. WHAT GNU od -t a MAKES OF THOSE THREE BYTES
   ef & 7f = 6f   o
   bb & 7f = 3b   ;
   bf & 7f = 3f   ?
   GNU od -t a names the low seven bits of a byte, so a file that starts
   with a BOM starts, under -t a, with o ; ? -- three ASCII characters
   that are nowhere in it. BSD od prints ef bb bf instead. A listing that
   opens with o ; ? is GNU output of a file with a BOM.

3. THE LEADING ZERO MAKES -j OCTAL, NOT THE ADDRESS COLUMN

$ od -Ad -j0400 -N8 -tc digits.txt | tidy
0000256 8 5 0 8 6 0 8 7
0000264

$ od -Ad -j400 -N8 -tc digits.txt | tidy
0000400 3 3 1 3 4 1 3 5
0000408

$ od -Ad -j0x190 -N8 -tc digits.txt | tidy
0000400 3 3 1 3 4 1 3 5
0000408
   -j and -N read a number the way C does: a leading 0 is octal, 0x is
   hex, anything else is decimal. The offset column is DECIMAL here
   (-A d), and -j0400 still started at byte 256, in the middle of 085.
   So -j0000640 is octal because of its first zero, and -j0640 is the same
   request. It matches od's address column only because that column
   happens to be octal by default.

4. THE FIRST WORD OF AN ELF FILE, READ BACKWARDS

$ od -An -tx1 elf.bin | tidy
...
0000010
   -t x with no size is an int: four bytes, printed as one number in this
   CPU's byte order. Little-endian puts the low byte first, so the magic
   7f 45 4c 46 -- DEL E L F -- comes out as 464c457f. Bare od is -t o2,
   and 7f 45 becomes the 16-bit value 0x457f, which is 042577 in octal.
   A dump of /bin/echo on 64-bit Linux opens with these same two lines.
```
<!-- /output -->

**1. The three bytes before the title are a byte order mark.** The chapter's `-t c` listing of the Gutenberg file opens with `357 273 277`, then the first letter of the title, and the text passes over them. They are octal for `ef bb bf`, the UTF-8 encoding of U+FEFF. A program that reads the file as plain ASCII sees a first line that begins with three extra bytes, which is the whole failure in [A BOM in a CSV](../../07_Real_Data/bom_in_a_csv/README.md). What the mark is for, and when a file should carry one, is [Byte order and the BOM](../../03_Encodings/byte_order_and_bom/README.md).

**2. `o ; ?` comes from GNU `od` masking the high bit, not from the file.** Under `-t a` the same file opens `o ; ?`. GNU `od` drops the top bit of each byte and names the seven bits that are left, so `ef bb bf` turn into `6f 3b 3f`. A Mac prints the bytes instead:

```text title="Measured 2026-09-13 — macOS 26.6.2 (BSD od) and ubuntu:24.04 (od GNU coreutils 9.4), book.txt built as in section 1 of the run above, LC_ALL=C. Verbatim, through cat -vet; not machine-checked, because the two print different names for the same bytes."
# macOS 26.6.2, BSD od
$ od -ta book.txt | cat -vet
0000000   ef  bb  bf   C   h   a   p   t   e   r  cr  nl  ht   I   n   d$
0000020    e   n   t   e   d  cr  nl                                    $
0000027$

# ubuntu:24.04, od (GNU coreutils) 9.4
$ od -ta book.txt | cat -vet
0000000   o   ;   ?   C   h   a   p   t   e   r  cr  nl  ht   I   n   d$
0000020   e   n   t   e   d  cr  nl$
0000027$
```

So a `-t a` listing that opens `o ; ?` was printed by GNU `od`, from a file that starts with a BOM. [The two rows to avoid](#the-two-rows-to-avoid) is the rule, and this is the rule broken in print.

**3. The leading zero makes `-j0000640` octal, not the address column.** The book writes its `-j` and `-N` values with leading zeros, like the address column, and warns that a bare `640` would be read as decimal. The warning is right, but the reason is off. `-j` and `-N` read a number the way C does: a leading `0` means octal, `0x` means hex, and anything else is decimal. That holds whatever `-A` has done to the address column. In section 3 of the run, the column is decimal and `-j0400` still starts at byte 256. `-j0640` is the same request as `-j0000640`, because only the first zero counts.

**4. `464c457f` is `7f 45 4c 46` read backwards.** The chapter's `od -tx /bin/echo` opens with `464c457f`, and the text does not say why it reads that way. `-t x` with no size means an `int`, so `od` takes four bytes at a time and prints each group as one number in the machine's byte order. The file itself starts `7f` `E` `L` `F`, the [ELF](../../16_Formats/elf/README.md) magic number. That is [the type is the interface](#the-type-is-the-interface) again: a dump you are reading for bytes wants `-t x1`.

**5. A carriage return breaks the shebang, and the rest of the script still runs.** The chapter ends by saying the operating system cannot read a script or configuration file that contains carriage returns. It also says Windows editors add a newline and a carriage return, while its own listing shows the order in the file is `\r \n`, CR first. What really happens is narrower, and worse, because nothing stops. `./script` fails at `#!` because the interpreter's name ends in a CR, which is [The first two bytes](../../06_Terminal/the_first_two_bytes/README.md). `bash script` runs every line with a CR stuck to its last word. Measured on both machines on 2026-09-13, a `cd /tmp` line failed because `/tmp` plus a CR is not a directory, the next line ran anyway, and the script exited 0. An `if` block stops the script with a syntax error instead ([CRLF vs LF](../../07_Real_Data/crlf_vs_lf/README.md)). A configuration file is not read by the operating system at all; whether its CRs matter is up to the one program that parses it. That leaves the chapter's own advice, which is good advice: look with `-t c`.
...
<!-- output:od_kata_sh -->
*Verified output of [`od_kata_sh.sh`](examples/od_kata_sh.sh) — regenerated by `tools/run_examples.py`, never hand-typed.*
