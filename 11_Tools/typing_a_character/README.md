...
The fourth is the one people do not know they have: it is a keyboard *layout*, so you add it in Keyboard Settings and switch to it, and then `Option` plus `20AC` is a euro. It reads four hex digits per character, so it reaches the BMP directly, and a code point above `U+FFFF` — `😀` is `U+1F600` — cannot be typed as its number. It can be typed as its two [UTF-16 surrogates](../../03_Encodings/utf16_and_surrogates/README.md), one four-digit sequence each: `Option`+`D83D`, then `Option`+`DE00`. [A user on macOS 10.13 reports ↗](https://apple.stackexchange.com/a/382647) building the flag 🇫🇷 exactly that way, from `D83C DDEB` and `D83C DDF7`. *Not re-verified here: adding the layout means changing System Settings, which this measurement did not do.* The planes themselves are in [Writing a code point](../../02_Characters/writing_a_code_point/README.md).
...
**Four ways to produce `ż`.** Name them — two are input methods belonging to specific programs, two identify the character itself. Say which two, and why only those two can appear in source code or a bug report.

Then: the X11 compose table and Vim's digraph table can produce 2,580 characters between them and agree on only 611. Say what that number means about a sequence you have memorised, and give the escape for `ż` in Python, in Rust, and in the shell — noting which of the three has no character escape at all.

<details markdown="1">
<summary><strong>Answers</strong></summary>

<!-- output:typing_a_character_kata_py -->
*Verified output of [`typing_a_character_kata_py.py`](examples/typing_a_character_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
target   ż   U+017C   LATIN SMALL LETTER Z WITH DOT ABOVE

FOUR WAYS TO TYPE IT
   ...
   Unicode name   LATIN SMALL LETTER Z WITH DOT ABOVE normative, and the only one a PROGRAM can use

THE TWO TABLES DISAGREE, AND THAT IS THE POINT
   Measured on this library's own parse of both files: the compose file
   and Vim's digraph table can produce 2,580 characters between them and
   agree on only 611. So a sequence you learned in one is usually not a
   sequence in the other, and neither is a property of the CHARACTER --
   they are properties of two programs.

THE NAME IS THE ONE THAT TRAVELS
   ud.lookup('LATIN SMALL LETTER Z WITH DOT ABOVE' ) -> ż
   ud.lookup('EURO SIGN'                           ) -> €
   ud.lookup('GRINNING FACE'                       ) -> 😀
   It is normative, it is stable for the life of the code point, and it
   is the same in every language that ships a Unicode table -- Python's
   \N{...}, Rust's char::from_u32 plus a lookup crate, and the name
   column of every reference. That is why it is what to write in source
   and in a bug report, and why 'the z with the dot' is not.

AND WHEN YOU CANNOT TYPE IT AT ALL
   The escape is always available, and its shape says what the language
   thinks a character is:
   Python  "\N{LATIN SMALL LETTER Z WITH DOT ABOVE}"  or  "\u017c"
   Rust    '\u{17c}'
   shell   printf '\xc5\xbc'   -- BYTES, not a character
   The shell has no character escape at all -- only bytes -- which is
   the same gap C has, and for the same reason.
```
<!-- /output -->
