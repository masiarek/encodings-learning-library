...
Two of grep's behaviours differ between **BSD grep**, which ships with macOS, and **GNU grep**, which ships with Ubuntu — both of them entries in [the list of platform splits this library records](../../CONTRIBUTING.md) — so no answer key can cover them and nothing above tries to. Both were measured on one file, on the two machines named in the caption.
...
| `grep -nP '[^\x00-\x7F]' f` | GNU only | BSD grep has no [`-P`](../pcre2/README.md) at all — it exits with a usage error |
...
<!-- output:grep_kata_sh -->
*Verified output of [`grep_kata_sh.sh`](examples/grep_kata_sh.sh) — regenerated by `tools/run_examples.py`, never hand-typed.*
