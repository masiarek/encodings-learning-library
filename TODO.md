...
| # | Page | Lands in | The hook to verify |
|---|---|---|---|
| 23 | Bytes that are not text — `surrogateescape` | 04 | [PEP 383 ↗](https://peps.python.org/pep-0383/) round-trips an undecodable filename *through* a `str`. The mechanism behind `os.fsdecode`, and the honest answer to "but the filename isn't UTF-8". **Partly shown already** on ["Handles Unicode" is four questions](10_Best_Practices/what_your_language_gives_you/README.md), which uses the round trip as one language's answer to one of four questions and pairs it with the web standard's independent reinvention of the same trick (`x-user-defined`, private use area instead of lone surrogates) — the dedicated page still owes `os.fsdecode`, `sys.getfilesystemencoding` and the Windows half |
