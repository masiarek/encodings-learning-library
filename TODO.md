...
| 17 | ~~Opening a file~~ | 04 | **Written 2026-09-07** — [Opening a file](04_Python/opening_a_file/README.md). The trap was real and is disclosed on the page: the harness sets `PYTHONUTF8=1`, which *is* UTF-8 Mode, so every default is read from a child interpreter instead. `io.text_encoding()` and PEP 597's `EncodingWarning` both earned a section. Two values were kept out of the key for being facts about the runner — the C locale's name (`US-ASCII` / `ANSI_X3.4-1968`) and `sys.getfilesystemencoding()`, `utf-8` on macOS against `ascii` on glibc, the Python-side face of [`find`'s](11_Tools/find/README.md) APFS finding |
...
| 30 | Case is not a per-character operation | 02 | `ß` uppercases to `SS`, so the string gets longer — which is why Rust's `to_uppercase` returns an iterator. Then Turkish `I`, and Greek final sigma, where case depends on locale and on position. **Narrowed 2026-09-08:** ["Supports Unicode" is a level, not a yes](02_Characters/what_a_regex_matches/README.md) measured the *matching* half — `re.IGNORECASE` puts `i`, `I`, `İ` and `ı` in one class where `str.casefold()` makes three, because `re` closes over case mappings rather than case folding. What is left here is the *transformation* half: the length change, Rust's iterator, and the locale |
...
**And one item that belongs to a different library.** Adam asked for two Perl tutorials to be followed. [`perlpacktut` ↗](https://perldoc.perl.org/perlpacktut) became [Packing a record](07_Real_Data/packing_a_record/README.md) on 2026-09-08. The other, [`perlfaq5` ↗](https://perldoc.perl.org/perlfaq5) — file I/O: opening, locking, slurping, `$/`, in-place edit, temporary files — is **still open and is not this library's**: it belongs against the Python library's `opening_a_file` stub, where the subject is files rather than encodings. This library's own [Opening a file](04_Python/opening_a_file/README.md) covers the `encoding=` half only, and should stay that narrow.
