...
- `surrogateescape` gets a mention and not a section — the mechanism, the `surrogatepass` confusion and where it leaks are already done in [Bytes that are not text](../surrogateescape/README.md), so this page shows it as one column of the five and links there
