...
- **The locale lesson's UTF-8 half.** Only recordable if both CI runners have a UTF-8 locale installed; check `locale -a` on each before promising it. **Partly unblocked 2026-09-06:** the point survives on the C locale alone — `wc -m` says 6 where Python's `len()` says 5 on the same `café\n`, because UTF-8 mode enables itself under `C`/`POSIX` (PEP 540) while `wc` obeys the locale. Reproduced on macOS 3.14.2 and `python:3.12-slim`; written into the stub.
