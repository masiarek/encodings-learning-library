...
- **~~How you type it.~~ Half of it written 2026-09-06** as [Writing a code point](02_Characters/writing_a_code_point/README.md), which took the source-code half: `'\N{EURO SIGN}'` in Python, `'\u{20AC}'` in Rust, the fixed widths against the braces, and C's universal character name. What it did **not** take is the half about your hands rather than your source — `%(keysym)` and `%(digraph)`, the compose key, Vim digraphs, and `uni print U+20AC | pbcopy`. That is still a short page, and its natural home is beside [`uni`](11_Tools/uni/README.md) rather than in chapter 2, since every one of those is a tool and not a language.
