...
- **The Unicode character encoding model (UTR #17)** — five layers, and the list runs three of them together: abstract character repertoire → coded character set → character *encoding form* (code units) → character *encoding scheme* (bytes, hence byte order) → transfer encoding syntax. **"UTF-16" names a form; "UTF-16LE" names a scheme.** Half the BOM confusion is this distinction missing.
