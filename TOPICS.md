...
|---|---|---|
| *"Unicode stability policy — assigned characters will never be removed **or have their properties changed**"* | **185 code points** changed `General_Category` between UCD 3.2 and 16.0. `U+00AD SOFT HYPHEN` went `Pd` → `Cf`; 85 went `Lo` → `Lu` | Half true, and the false half is the load-bearing one. **Name** and non-reuse are guaranteed; **General_Category is frozen only for control, private-use and surrogate code points** (`Cc`, `Co`, `Cs`), so a letter's or a soft hyphen's can move. This is the distinction [The table has a version](02_Characters/the_table_has_a_version/README.md) is built on |
...
*Covered:* [Hex is a shorthand](01_Bits_and_Bytes/hex_is_a_shorthand/README.md) · [Which base did you mean?](01_Bits_and_Bytes/which_base_did_you_mean/README.md) · [Hex: a number, or a picture of bytes](01_Bits_and_Bytes/hex_number_or_bytes/README.md) · [Reading a hex dump](01_Bits_and_Bytes/reading_a_hex_dump/README.md) · [A byte is eight bits](01_Bits_and_Bytes/a_byte_is_eight_bits/README.md) · [Why hex and not octal](01_Bits_and_Bytes/hex_is_a_shorthand/README.md#why-hex-and-not-octal) · [Writing the literal](01_Bits_and_Bytes/hex_is_a_shorthand/README.md#writing-the-literal) · [A byte was not always eight bits](01_Bits_and_Bytes/a_byte_is_eight_bits/README.md#a-byte-was-not-always-eight-bits)
...
- ~~**How a number gets INTO a program that has no compiler**~~ — written 2026-09-12 as [Which base did you mean?](01_Bits_and_Bytes/which_base_did_you_mean/README.md), which is the third direction the two literal bullets above leave out: not how a byte is shown and not what a compiler accepts in source, but what a *field* does with digits nobody labelled. Place value stated once for any base, the four notations for naming one (`0x41` · `41h` · `16#41` · `X'41'`) and the fifth that names none; `[ 010 -eq 10 ]` and `[[ 010 -eq 8 ]]` both true in one shell; a dump whose offset column is hex and an `xxd -s` that reads its argument as C would; `dd` disagreeing with itself across platforms in both directions; and `socket.inet_aton` taking `010.0.0.1`, `0x8.0.0.1` and `134744072` where `ipaddress` refuses all three. The mechanism under all of it is one `int` argument — `strtol`'s base 0 means *read the prefix* — which is what the tools that disagree are disagreeing about.
...
- **`PYTHONIOENCODING`, `PYTHONUTF8`, PEP 540 UTF-8 mode, PEP 538 locale coercion, PEP 686** — the five levers that decide what Python's streams do. The runner pins the environment for exactly this reason — and for a Python example it is `LC_ALL=C` that does the pinning, not the `PYTHONUTF8=1` beside it: the example runs as `python3 -I`, which ignores every `PYTHON*` variable, and PEP 540 switches UTF-8 Mode on under a C locale by itself ([Opening a file](04_Python/opening_a_file/README.md#this-pages-own-environment-is-rigged-and-says-so) measures both halves).
...
*Covered:* [04_Python](04_Python/README.md) · [05_Rust](05_Rust/README.md) · [Python text in practice](10_Best_Practices/python_text_in_practice/README.md) · [Rust strings in practice](10_Best_Practices/rust_strings_in_practice/README.md) · [`rune` is an `int32`](02_Characters/rune_is_an_int32/README.md) (Go's `rune`, .NET's `Rune`)
