...
**ABAP.** `utf-8-sig` is an addition here rather than a codec: `OPEN DATASET … IN TEXT MODE ENCODING UTF-8 SKIPPING BYTE-ORDER MARK` [steps over a mark at the start of the file if there is one ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapopen_dataset_encoding.htm), and `WITH BYTE-ORDER MARK`, on a file opened for output, writes one. SAP's own advice is to add `SKIPPING` to every UTF-8 read — and, the opposite of the Python rule above, to write text files *with* a mark, so a file from a program that follows it starts with `EF BB BF`, which is exactly what `utf-8-sig` is for on the Python side. Leave `SKIPPING` off and the mark is read as data, so an inbound file from a Windows or Excel producer arrives with `EF BB BF` welded to the first field of the header record — and the symptom is an interface whose first column never matches while every other column is fine, which reads like a mapping bug and is not one. Bytes that reach you some other way — an upload, an `xstring` from an interface — never pass through that addition, so check the first record and strip the mark there. `cl_abap_char_utilities` is where the byte-order-mark constants live on most releases; check what your system actually offers rather than trusting a name from a document, and see [SAP code pages](../../07_Real_Data/sap_code_pages/README.md) for the wider habit. *(Not machine-checked — CI cannot run ABAP.)*
...
**Write the bytes.** Predict, byte for byte, what `"ID"` becomes in `utf-8`, `utf-8-sig`, `utf-16`, `utf-16-le`, `utf-16-be` and `utf-32`. Which of the six carry a mark, and how long is each mark?

Then the bug this page exists for. A CSV starts `ef bb bf 49 44 2c ...`. Read it as `utf-8` and as `utf-8-sig`, and in each case say what the first column is *called* — then explain why `row['ID']` raises `KeyError` while the header looks perfectly correct in the error message.

<details markdown="1">
<summary><strong>Answers</strong></summary>

<!-- output:byte_order_and_bom_kata_py -->
*Verified output of [`byte_order_and_bom_kata_py.py`](examples/byte_order_and_bom_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
encoding     bytes                        len  mark?
utf-8        49 44                          2
utf-8-sig    ef bb bf 49 44                 5  signature EF BB BF
utf-16       ff fe 49 00 44 00              6  BOM FFFE
utf-16-le    49 00 44 00                    4
utf-16-be    00 49 00 44                    4
utf-32       ff fe 00 00 49 00 00 00 44 00 00 00  12  BOM, 4 bytes of it

utf-16 with no suffix WRITES A BOM and picks an order for you; the -le
and -be forms write neither, because you already said which. That is the
whole job of the mark: a value wider than one byte has to go into the
file in some order, and U+FEFF first lets the reader work out which.

UTF-8 HAS NO ORDER TO RESOLVE, AND GETS THE MARK ANYWAY
   utf-8      49 44
   utf-8-sig  ef bb bf 49 44   <- the same three bytes, every time
   Bytes cannot be in the wrong order when the unit IS a byte. Those
   three bytes are a SIGNATURE -- a label saying 'this is UTF-8' -- and
   Windows tools write it so Notepad can tell UTF-8 from a code page.

AND HERE IS THE CSV BUG, IN FULL
   file starts   ef bb bf 49 44 2c 6e 61
   read as utf-8      first column '\ufeffID'
   read as utf-8-sig  first column 'ID'
   equal to 'ID'?     False and True

   The first column is not called ID. It is called ZERO WIDTH NO-BREAK
   SPACE followed by ID, and it draws as 'ID' on every screen you will
   look at. row['ID'] raises KeyError, the header LOOKS right in the
   error message, and this is the single most reported bug in this
   chapter. The fix is to name the encoding utf-8-sig when reading a
   file that might have come from Excel -- it strips the mark if it is
   there and does nothing if it is not.
```
<!-- /output -->
