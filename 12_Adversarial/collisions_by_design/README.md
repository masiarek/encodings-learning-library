...
**Six signups.** Run `admin`, `ADMIN`, `Admin`, `ａdmin`, `ⓐdmin` and `admın` through NFKC + casefold and write down the equivalence classes.
...
<!-- output:collisions_by_design_kata_py -->
*Verified output of [`collisions_by_design_kata_py.py`](examples/collisions_by_design_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
SIX SIGNUPS, AND THE CLASSES THEY FALL INTO
   'admin'    <- ['admin', 'ADMIN', 'Admin', 'ａdmin', 'ⓐdmin']
   'admın'    <- ['admın']
