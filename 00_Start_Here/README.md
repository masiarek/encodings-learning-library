...
| [02_Characters](../02_Characters/README.md) | A character is a number by agreement — 128, then 256 with everybody's own top half, then one numbering for all | 19 / 1 |
...
| [11_Tools](../11_Tools/README.md) | `grep`, `find`, `sort`, `tr` — none of them is *about* encodings, and every one has already made an encoding decision for you without saying so | 24 / 0 |
...
| [16_Formats](../16_Formats/README.md) | Ghidra's import dialog, one page per format: where each of twenty-four headers keeps the answers a raw byte stream does not give, and the three that make the reader choose | 24 / 0 |
