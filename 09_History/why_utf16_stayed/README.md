...
**ABAP is still UCS-2 today, and that is the sharpest way to say what it inherited.** SAP separates the two halves and the separation is load-bearing: the *system code page* of a Unicode system is UTF-16, but [the ABAP language supports UCS-2 ↗](https://help.sap.com/doc/abapdocu_751_index_htm/7.51/en-us/abenucs2_glosry.htm) — "mainly UTF-16 without surrogates," in SAP's own gloss — and therefore [interprets a surrogate pair as two characters ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abensurrogates_glosry.htm). So `strlen( )` over an emoji does return `2`, the same number Java and JavaScript return, **for a different reason**: their `2` means *one character stored in two units*, and ABAP's means *two characters*, because the pair was never assembled. The one function that does see the pair is `charlen( )`, documented as returning 1 for a single Unicode character and 2 for a surrogate pair.

That difference is invisible until something cuts. SAP names the failure precisely — trouble arises when a string is truncated in the middle of a surrogate representation, or when individual characters are compared — which is the same offset argument as above, arriving from the other end: the fixed-width guarantee that makes `record+8(3)` safe is exactly what makes `record+8(1)` able to slice a character in half, once a character stops being one unit.
...
**What did those four systems actually adopt?** Java, Windows, JavaScript and SAP all report an emoji's length as 2. Say what Unicode *was* when each of them adopted it, and why that made their choice correct at the time.

Then say why none of them can change it now — the reason is not technical difficulty. Finish with ABAP's second reason, which the other three do not have, and name three places you still pay for this today.

<details markdown="1">
<summary><strong>Answers</strong></summary>

<!-- output:why_utf16_stayed_kata_py -->
*Verified output of [`why_utf16_stayed_kata_py.py`](examples/why_utf16_stayed_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
WHAT UNICODE WAS WHEN THEY ADOPTED IT
   Until Unicode 2.0 (1996) the whole character set fit in 16 bits:
   65,536 code points, one 16-bit unit each, fixed width. A char was a
   number, an array of them was a string, and s[i] was O(1) and correct.
   Java (1995), Windows NT (1993), JavaScript (1995) and SAP all took
   that deal, and it was a GOOD deal.

THEN THE SET GREW, AND FIXED WIDTH BECAME A SURROGATE PAIR
   😀  U+1F600
   utf-16 units  2   bytes 4
   utf-8 bytes   4
   code points   1
   Every one of those four systems now reports this character's length
   as 2, because their unit is the 16-bit code unit and always was.

WHY NONE OF THEM CAN CHANGE IT
   The type is in every public API. Java's String.charAt, JavaScript's
   string indexing, the Windows W-suffixed API, ABAP's character
   fields -- changing the unit would change the meaning of every
   existing index in every program ever compiled against them. So the
   cost is not technical difficulty; it is that the width is part of a
   contract with thirty years of callers.

ABAP'S SECOND REASON, WHICH THE OTHER THREE DO NOT HAVE
   ABAP field offsets are ARITHMETIC: name+10(20) means 'twenty units
   starting at unit ten', and that appears in production code written
   against fixed-width structures. With a fixed-width encoding the
   arithmetic is a fact about the layout. With a variable-width one it
   would be a guess -- so UCS-2/UTF-16 is not merely SAP's historical
   choice, it is what makes an enormous body of existing code mean
   anything at all.

WHAT THIS COSTS YOU IN PRACTICE
   A VARCHAR(1) that rejects an emoji is counting code units.
   A JavaScript .length of 2 for one character is counting code units.
   A JSON escape of an emoji is TWO \u escapes, because \u names a code
   unit -- and that is how a surrogate pair reaches a format whose
   transport encoding, by RFC 8259, is UTF-8.
   json.dumps: '😀' -> "\ud83d\ude00"

THE LESSON THAT TRANSFERS
   Nobody chose UTF-16 over UTF-8. They chose a fixed-width 16-bit
   character set, correctly, before it stopped being one -- and then
   inherited a variable-width encoding they never signed up for. When
   you pick a text representation today you are making the same kind of
   bet, and the question to ask is what happens when the assumption
   underneath it stops holding.
```
<!-- /output -->
...
- [ABAP: UCS-2 ↗](https://help.sap.com/doc/abapdocu_751_index_htm/7.51/en-us/abenucs2_glosry.htm) and [ABAP: surrogate ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abensurrogates_glosry.htm) — where SAP says ABAP does not support surrogates and reads a pair as two characters
