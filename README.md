...
| [15_Hex](15_Hex/README.md) | What each entry in a hex editor's Hex Operations dialog does to the bytes — twenty operations, one page each, and the settings that decide which bytes they see |
| [16_Formats](16_Formats/README.md) | Ghidra's import dialog, one page per format — twenty-four headers asked the four questions a raw byte stream cannot answer: which end, how wide, where a string ends, which base — with the real files on this Mac in dated fences |
