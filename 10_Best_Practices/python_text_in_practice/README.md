...
**ABAP.** The nearest thing to `encoding=` is the `ENCODING` addition on `OPEN DATASET`, and in text mode it is [mandatory ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapopen_dataset_encoding.htm), so there the omission `-X warn_default_encoding` warns about is a syntax error. `ENCODING DEFAULT` is not the trap either — since 7.50 it means UTF-8 on every system, the change Python 3.15 makes to `open()`. On a non-Unicode system [7.40 still made it `NON-UNICODE` ↗](https://help.sap.com/doc/abapdocu_740_index_htm/7.40/en-US/abapopen_dataset_encoding.htm), which meant the system's own code page, unconverted. Name `UTF-8` anyway, for the reason `encoding=` stays in the call after 3.15. The locale-dependent encoding survives in two spellings the syntax check accepts: `ENCODING NON-UNICODE`, which is `encoding="locale"` — the same dependence, asked for by name — and `IN LEGACY TEXT MODE` or `IN LEGACY BINARY MODE` [with no `CODE PAGE` ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapopen_dataset_code_page.htm), which is `open()` with no `encoding=`. Both take the non-Unicode code page that table `TCP0C` assigns to the current text environment, which your logon language sets and [`SET LOCALE LANGUAGE` ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapset_locale.htm) changes, so one program can read a file correctly for you and wrongly for a colleague who logs on in another language. The audit is a grep for `NON-UNICODE`, and for `LEGACY` statements that name no `CODE PAGE`, done by hand. The `xstring` ↔ `string` conversions are `cl_abap_codepage=>convert_to` / `convert_from`, and `OPEN DATASET` eats a byte-order mark only when asked, with [`SKIPPING BYTE-ORDER MARK` ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapopen_dataset_encoding.htm), which belongs on every UTF-8 read; on any other route in, check what the conversion does with one rather than assuming. *(Not machine-checked — CI cannot run ABAP. Verify code-page numbers against your own system.)*
...
**Four habits.** Say what each prevents: passing `encoding=` to every `open()`; running the tests once under `-X warn_default_encoding`; treating filenames as bytes; and distrusting `bytes` methods that look like `str` methods.

For the filename habit, round-trip `b"caf\xe9.txt"` through `os.fsdecode`/`os.fsencode` and explain what makes it work and what it costs. For the last, predict `"CAFÉ".encode().lower()` and say why the answer is correct and still a bug.

<details markdown="1">
<summary><strong>Answers</strong></summary>

<!-- output:python_text_in_practice_kata_py -->
*Verified output of [`python_text_in_practice_kata_py.py`](examples/python_text_in_practice_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*
...
1. PASS encoding= TO EVERY open(), EVEN WHEN IT IS THE DEFAULT
   ...
   nothing in the traceback points at the open() call.

2. RUN THE TESTS ONCE UNDER -X warn_default_encoding
   python3 -X warn_default_encoding -W error::EncodingWarning -m pytest
   Every open() that relied on the default becomes a warning naming the
   file and line. It is a one-off audit, not a permanent setting, and it
   is the only way to find the ones you did not write.

3. FILENAMES ARE BYTES WEARING A COSTUME
   sys.getfilesystemencoding()      'utf-8'
   sys.getfilesystemencodeerrors()  'surrogateescape'
   os.fsdecode(b'caf\xe9.txt') -> 'caf\udce9.txt'
   os.fsencode(that)    -> b'caf\xe9.txt'   round trip: True
   The name is not valid UTF-8 and Python still round-trips it, because
   surrogateescape puts each undecodable byte in a lone surrogate. The
   cost: that str cannot be encoded by anything else, so printing it or
   putting it in JSON raises -- correctly.

4. bytes METHODS THAT LOOK LIKE str METHODS ARE ASCII-ONLY
   b'CAF\xc3\x89'.lower() -> b'caf\xc3\x89'
   'CAFÉ'.lower() -> 'café'
   The bytes version lowercased C, A, F and left the two bytes of É
   alone -- because bytes.lower() is defined on ASCII and nothing else.
   That is documented and it is on purpose: bytes have no encoding, so
   there is no other case mapping it could apply. It is still a silent
   wrong answer if you thought you were working with text.

THE ONE-LINE VERSION
   Say the encoding out loud at every boundary, treat filenames as a
   special case that Python has already solved for you, and never let a
   bytes object anywhere near a method whose name suggests it knows
   about language.
```
<!-- /output -->
