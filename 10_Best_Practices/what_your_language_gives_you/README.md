...
**Predict, then run.** Six conversions of two values — the Polish word `żółw` and the bytes `b'caf\xe9.txt'`, a Latin-1 `é` loose in a stream that was declared UTF-8. For each line write down which of three things happens — it **raises**, it **loses** something, or the value **comes back unchanged** — before running any of them:

```python
'żółw'.encode('cp1250').decode('cp1250')
'żółw'.encode('latin-1').decode('latin-1')
'żółw'.encode('latin-1', 'replace').decode('latin-1')
'żółw'.encode('latin-1', 'backslashreplace').decode('latin-1')
b'caf\xe9.txt'.decode('utf-8', 'surrogateescape').encode('utf-8', 'surrogateescape')
b'caf\xe9.txt'.decode('latin-1').encode('latin-1')
```

Three of the six come back unchanged, and one of those three is the one nobody predicts. When you have your six answers, say **which of the losses a monitoring system would catch** — then the second half, which needs no machine at all: of the six lines, which could a browser perform? Answer that before you open the fold, because it is the whole page in six lines.

```bash
bash 10_Best_Practices/what_your_language_gives_you/examples/what_your_language_gives_you_kata_sh.sh
```

<details markdown="1">
<summary><strong>Answers</strong></summary>

Every row of this key is the same on every machine. Nothing in it reads a username, a date or a locale, and it was diffed byte for byte against `ubuntu:24.04` and against CPython 3.11, 3.12, 3.13 and 3.14 before it was recorded — so a row that does not match on your machine is a finding, not a difference of setup.

<!-- output:what_your_language_gives_you_kata_sh -->
*Verified output of [`what_your_language_gives_you_kata_sh.sh`](examples/what_your_language_gives_you_kata_sh.sh) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
PART ONE -- SIX ROUND TRIPS
------------------------------------------------------------------------

   1  'żółw'.encode('cp1250')                              round-trips
   2  'żółw'.encode('latin-1')                             UnicodeEncodeError
   3  'żółw'.encode('latin-1', 'replace')                  LOSES  -> '?ó?w'
   4  'żółw'.encode('latin-1', 'backslashreplace')         LOSES  -> '\\u017có\\u0142w'
   5  b'caf\xe9.txt'.decode('utf-8', 'surrogateescape')    round-trips
   6  b'caf\xe9.txt'.decode('latin-1')                     round-trips

   the word, as characters       U+017C U+00F3 U+0142 U+0077
   which of them latin-1 holds   nyny   (2 of 4)
   line 1 wrote                  bf f3 b3 77
   line 3 wrote                  3f f3 3f 77   = '?ó?w'
   line 4 wrote                  14 bytes: \u017có\u0142w
   line 5 held the byte as       U+DCE9
   line 6 held the byte as       U+00E9
...
PART TWO -- WHICH OF THE SIX COULD A BROWSER DO?
------------------------------------------------------------------------
...
- [Bytes that are not text](../../04_Python/surrogateescape/README.md) — question 2 in full: `os.fsdecode`, the filesystem encoding, and the Windows half
