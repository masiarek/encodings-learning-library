...
This is the part the number reading hides most completely. `0041` as a *number* is 65 on every machine ever built. As *bytes* it is 65 or 16,640, depending on a convention the digits do not record — which is why `u32::to_be_bytes` / `to_le_bytes` in Rust are two different methods rather than one with a flag, and why Python's answer is worth a second look: `int.to_bytes` and `int.from_bytes` took a **required** byte order until 3.10, and since 3.11 default it to `'big'` — which is not what `sys.byteorder` reports on any machine you are likely to own. The question does not go away when a language gives it a default; it just stops being asked out loud. [The bytes do not say which end](../which_end_comes_first/README.md) takes all of it apart, and [byte order and the BOM](../../03_Encodings/byte_order_and_bom/README.md) is the same question where the thing being ordered is a character. The point here is only that it comes into existence at the moment you cross from one reading to the other, and did not exist before.
...
**ABAP** *(Not machine-checked — CI cannot run ABAP.)* ABAP is unusual in having chosen the *byte* reading as the default: `TYPE x` and `xstring` are written and displayed as hex, two digits per byte, always, and the leading zeros are simply part of the value — `DATA b TYPE x LENGTH 2 VALUE '0041'.` is two bytes and stays two bytes. The number door is the explicit one, and it is where the care is needed: assigning an `x` field to an integer type reads it [as big-endian on every server ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abenconversion_type_x.htm), not in the platform's own byte order, which is what an `ASSIGN … CASTING` sees instead — so a little-endian field from an interface becomes a wrong number with no error. A hex field that arrived from an interface should be read as bytes and only turned into a number when something genuinely needs its magnitude; [Bytes, hex and int](../../04_Python/bytes_hex_and_int/README.md#if-you-are-coming-from-python-or-abap) has the whole rule. `cl_abap_conv_codepage` and the `xstrlen( )` / `strlen( )` pair are the tools; the rule to carry over is the same as everywhere else on this page — decide which of the two objects the field is, in writing, before the first conversion.
...
**The field that arrived one character short.** An interface sends a 32-character field described as *"the hash, in hex"*. Today it arrives with 31 characters.

Say what each of the three readings does with it — `int(s, 16)`, `bytes.fromhex(s)`, and `xxd -r -p` — which one you want, and where in your program the check belongs. Then say why **one** length check is not enough.

<details markdown="1">
<summary><strong>Answers</strong></summary>

<!-- output:hex_number_or_bytes_kata_py -->
*Verified output of [`hex_number_or_bytes_kata_py.py`](examples/hex_number_or_bytes_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
the field   'd131dd02c5e6eec4693d9a0698aff95'   (31 characters, described as '32 hex')

1. AS A NUMBER -- int(s, 16)
   accepted, value 0xd131dd02c5e6eec4693d9a0698aff95
   A number has no width, so 31 digits is not short, it is just smaller.
   Nothing is missing from a number's point of view. Notice what the
   round trip does: it comes back with a LEADING ZERO restored in a
   place the sender never had one -- 0d131dd02c5e6eec4693d9a0698aff95
   That is the corruption, and it looks like a repair.

2. AS BYTES -- bytes.fromhex(s)
   ValueError -- refused, before a single byte is built
   The width IS the data here, and 31 does not divide by 2, so Python
   refuses. This is the reading you want: it is the only one of the
   three that knows a hash is 16 bytes rather than a large integer.

3. AS BYTES, THE SHELL'S WAY -- xxd -r -p
   Silently drops the last nibble and exits 0. The page's own one-liner
   shows it: printf '123' | xxd -r -p | xxd -p prints 12.
   Fifteen and a half bytes are not representable, so it writes fifteen
   and says nothing. A pipeline reading its status sees success.

WHERE THE CHECK GOES
   At the boundary, before the value becomes an object -- the same place
   every decode belongs. Two lines, and they are not the same check:
       if len(field) != 32: reject      # the width, which is the contract
       raw = bytes.fromhex(field)       # the alphabet, which is the parser's
   The second one alone would pass a 30-character field. The first alone
   would pass 32 characters of Arabic-Indic digits, which int() accepts
   and which are not hex at all: int('\u0664\u0661', 16) == 65.
```
<!-- /output -->
