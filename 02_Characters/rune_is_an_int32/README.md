...
| .NET | 5.0.5 | `Rune.IsValid(0xD800)` is `False`; `new Rune(0xD800)` throws `ArgumentOutOfRangeException`; a `Rune` is 4 bytes and a `Rune?` 8; `Encoding.UTF8.GetString` counts with the majority; and on 2026-09-11, `Rune.DecodeFromUtf8` stops after one byte of `65 cc 81`, and a `Rune` and a `string` sort `U+FF61` and `😀` in opposite orders |
...
**And .NET is still building on it.** The issue asking for `Rune` to be threaded through the string APIs, [dotnet/runtime#27912 ↗](https://github.com/dotnet/runtime/issues/27912), was filed in November 2018, a year before the type shipped, and its overloads arrive in .NET 11 — in preview as this is written — as about three dozen members of `String`, `StringBuilder`, `TextWriter` and `TextInfo` that take or hand back a `Rune`: `Split`, `IndexOf`, `Replace`, `Trim`, `Append`. Until then, looking for `😀` in a `string` means looking for a two-`char` substring. .NET 10's one addition to the type itself is a parser, [`IUtf8SpanParsable<Rune>` ↗](https://github.com/dotnet/runtime/pull/105773), and its whole body is `DecodeFromUtf8` plus a check that no byte is left over — so it accepts exactly one scalar value. `c3 a9` parses; the same `é` written as `e` plus U+0301, `65 cc 81`, does not, because the decoder stops after the `e` and one byte is not all three. (The decoder's answers were measured on .NET 5.0.5; the parser was read from the `release/10.0` source, since this Mac has no .NET 10.) .NET 11 adds the same parse from a `string`, and casing that matches `OrdinalIgnoreCase` — [Case is not a per-character operation](../case_is_not_per_character/README.md) has why that needed a name. And having both types puts two orders in one runtime: a `Rune` compares by code point and a `string` by UTF-16 code unit, so `A`, `😀`, `U+E000` and `U+FF61` sort as `A U+E000 U+FF61 😀` when they are `Rune`s and as `A 😀 U+E000 U+FF61` when they are strings — [the sort that is not code-point order](../../09_History/why_utf16_stayed/README.md), inside one program.
