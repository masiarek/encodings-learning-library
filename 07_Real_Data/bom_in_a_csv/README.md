...
**ABAP.** The inbound version of this is an interface whose *first field never matches* while every other field is fine — a Windows or Excel producer sends a file with `EF BB BF`, `OPEN DATASET … IN TEXT MODE ENCODING UTF-8` reads it as data, and the header record's first column arrives three bytes long. It reads like a mapping error and is not one. The fix is one more addition on the statement that named the encoding: `ENCODING UTF-8 SKIPPING BYTE-ORDER MARK` [steps over the mark when it is there ↗](https://help.sap.com/doc/abapdocu_750_index_htm/7.50/en-US/abapopen_dataset_encoding.htm), which is what `utf-8-sig` does, and SAP recommends it on every UTF-8 read. Bytes that arrive some other way — an upload, an `xstring` from an interface — never pass through it, so strip those on the first record only, at the boundary. And when writing a file for a user to open in Excel rather than for another system to parse, `WITH BYTE-ORDER MARK` writes the signature, and it is a deliberate choice worth a comment in the code, because the next person to touch it will assume it is a bug. Check what your release actually offers for the byte-order-mark constants rather than trusting a name from a document. *(Not machine-checked — CI cannot run ABAP.)*
...
**Three bytes, both the fix and the bug.** Read a BOM'd CSV as `utf-8` and as `utf-8-sig`, and a plain one both ways too — four combinations. Predict the first column's name in each, and which of the four would make `row['ID']` raise.

Then print the broken key's code points and say why it looks correct in the `KeyError` message. Finish with the part that makes this not simply a bug: name the consumer for which the BOM is required, and say what that means for how you write the file.

<details markdown="1">
<summary><strong>Answers</strong></summary>

<!-- output:bom_in_a_csv_kata_py -->
*Verified output of [`bom_in_a_csv_kata_py.py`](examples/bom_in_a_csv_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
without BOM  49 44 2c 6e 61 6d 65 0a ...
with BOM     ef bb bf 49 44 2c 6e 61 ...

READ THE BOM'D FILE FOUR WAYS
   BOM'd  read as utf-8      first key '\ufeffID'   row['ID'] would raise KeyError
   plain  read as utf-8      first key 'ID'         row['ID'] would work
   BOM'd  read as utf-8-sig  first key 'ID'         row['ID'] would work
   plain  read as utf-8-sig  first key 'ID'         row['ID'] would work

   utf-8-sig is the only reading that works on BOTH files: it strips the
   mark if it is there and does nothing if it is not. That is the whole
   recommendation for reading a CSV that might have come from Excel.

WHAT THE BROKEN KEY ACTUALLY IS
   repr          '\ufeffID'
   code points   U+FEFF U+0049 U+0044
   it prints as  ﻿ID   -- indistinguishable from ID on any screen
   U+FEFF is ZERO WIDTH NO-BREAK SPACE. It draws nothing, it is not
   whitespace to strip(), and it makes the header look perfectly correct
   in the KeyError message that mentions it.

AND NOW THE OTHER DIRECTION, WHICH IS WHY THIS IS NOT SIMPLY A BUG
   Excel on Windows reads a UTF-8 CSV correctly only if the BOM is
   there; without it, it falls back to the system code page and the
   accented names arrive as mojibake. So the same three bytes are:
       the FIX      for a person double-clicking the file
       the BUG      for the program parsing it
   There is no setting that is right for both, which means the skill is
   not 'strip BOMs' -- it is knowing which of the two consumers a given
   file is for, and writing it accordingly.

WRITING, THE TWO WAYS
   open(p,'w',encoding='utf-8')      -> 49 44 2c ...  for programs
   open(p,'w',encoding='utf-8-sig')  -> ef bb bf  for Excel
```
<!-- /output -->
