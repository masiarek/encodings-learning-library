...
**Totals right, keys matching nothing.** Take the same two-row CSV with CRLF and with LF endings. Predict, for each, what the last field's value is as a *string*, and what `float()` makes of it.

The arithmetic is identical and correct. Say where the extra byte went, why `id == '1'` is False on one of them, and why nothing raises anywhere. Then give three fixes in order of preference — and name the one popular method that is *not* a fix, and what it does instead.

<details markdown="1">
<summary><strong>Answers</strong></summary>

<!-- output:crlf_vs_lf_kata_py -->
*Verified output of [`crlf_vs_lf_kata_py.py`](examples/crlf_vs_lf_kata_py.py) — regenerated by `tools/run_examples.py`, never hand-typed.*
