...
Worth knowing before you reach for either: on the two builds measured here, **BSD `sed` understands `\r` in both the pattern and the replacement**, which the older folklore says it does not. Measured 2026-09-07 on BSD sed (Darwin 25.6) and GNU sed 4.9 — `s/\r$//` strips a CR and leaves a word ending in the letter `r` alone on both, and `s/$/\r/` appends one on both. `tr -d '\r'` sidesteps the question entirely and is the safer thing to write down for somebody else.
